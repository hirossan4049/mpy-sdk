...
// 書き込み先パス
const WRITE_PATH = "/flash/main.py";
...
`);
      ...
      await adapter.writeFile(WRITE_PATH, sampleCode);
...
app_files = os.listdir('/flash/app')
print('Files in /flash/app:', app_files)
if 'main.py' in app_files:
    print('main.py exists in /flash/app/')
    with open('${WRITE_PATH}', 'r') as f:
        content = f.read()
        print('File size:', len(content), 'bytes')
        print('First 100 chars:', repr(content[:100]))
else:
    print('main.py not found in /flash/app/')
...
`);
    ...
    try {
      const flashedContent = await adapter.readFile(WRITE_PATH);
...
`);
    ...
    console.log(`🔄 To restart: Power cycle the M5Stack (auto-runs from ${WRITE_PATH})`);
...
The sample code will be flashed to ${WRITE_PATH} and run automatically.
`);
}
